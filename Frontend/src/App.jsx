...
export default function App() {
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      case 'dashboard':
        return <DashboardView phases={phases} aiLogs={aiLogs} queries={queries} />;
      case 'fases':
        return (
          <PhasesView 
            phases={phases} 
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          ...
          <p className="text-xs text-slate-500 mt-2">Bases: Scopus, WoS, IEEE</p>
        </div>
      </div>
      ...
      <div className="bg-indigo-50 border border-indigo-100 p-6 rounded-xl">
        ...
        <p className="text-indigo-700 text-sm">
          Aquí podrás registrar cada paso de tu proyecto de grado. Usa el menú lateral para actualizar tus fases, generar consultas con Gemini IA, almacenar tus ecuaciones de búsqueda para Scopus y gestionar tus recursos bibliográficos.
        </p>
      </div>
    </div>
  );
