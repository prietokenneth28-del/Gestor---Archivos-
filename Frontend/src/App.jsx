...
function AILogsView({ logs, onAddLog, onUpdateLog, onDeleteLog }) {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [modalMode, setModalMode] = useState('add');
  const [submitting, setSubmitting] = useState(false);
      ...
      <div className="space-y-6">
        ...
        {logs.length === 0 && (
          <div className="p-8 text-center text-slate-500 border border-dashed border-slate-300 rounded-xl bg-slate-50/50">
            No hay registros de IA guardados. ¡Añade tu primera consulta!
          </div>
        )}
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-4 sm:p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <h3 className="text-lg font-semibold text-slate-800">
                {modalMode === 'add' ? 'Registrar Interacción con IA' : 'Editar Registro de IA'}
              </h3>
              <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-slate-600 transition-colors">
                <X className="w-5 h-5" />
              </button>
            ...
            <div className="p-4 sm:p-6 overflow-y-auto">
              <form id="ai-log-form" onSubmit={handleSubmit} className="space-y-4">
                <div>
